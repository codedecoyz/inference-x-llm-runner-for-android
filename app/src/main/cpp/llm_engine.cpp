      ...
      layer_cache_(6)  // keep 6 layers hot in memory
{
    ...
    try {
        Ort::ThrowOnError(OrtSessionOptionsAppendExecutionProvider_Nnapi(session_opts_, 0x001 /* FP16 */));
        LLOGI("NNAPI execution provider enabled");
    } catch (...) {
        LLOGI("NNAPI not available, using CPU fallback");
    }
...
std::vector<float> LLMEngine::run_layer(const std::string& layer_path,
    ...
    size_t kv_size = (size_t)config_.num_kv_heads * kv_seq * config_.head_dim;
